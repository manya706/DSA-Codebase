...
class Solution{
public:
    Node* build(int pre[], int size, int &id, int mini, int maxi){
        if(id==size || pre[id]>maxi || pre[id]<mini){
            return NULL;
        }
        Node* root = newNode(pre[id++]);
        root->left = build(pre,size,id,mini, root->data);
        root->right = build(pre,size,id,root->data, maxi);
        return root;
    }
    ...
    Node* post_order(int pre[], int size)
    {
        ...
        int id=0;
        Node* root=build(pre,size,id,mini,maxi);
        return root;
